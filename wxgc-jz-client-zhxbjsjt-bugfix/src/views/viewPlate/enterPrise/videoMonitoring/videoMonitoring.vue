          ...
          <div class="icon-document">
            <div class="videoScreen" style="height:100%">
              <videoScreen
                ...
                @fillLunBoActive="fillLunBoActive($event)"
              ></videoScreen>
            </div>
          </div>
        </com-line>
...
export default {
  ...
  methods: {
    ...
    fillLunBoActive(obj){
      // this.$refs.spEcharts.fillLunBoActive(obj);
    },
    reloadPlayerOptions(obj) {
      // 填充 active
      console.log('reloadPlayerOptions',obj)
      this.$refs.videoScreenBox.reloadPlayerOptions(obj);
    },
    closeName(number) {
      // 调用视屏列表方法spEcharts
