    ...
    >
      <el-table-column label="文件名" align="center"> 
         <template slot-scope="scope">
           {{handlepath(scope.row.otherFilePath)}}
        </template>
      </el-table-column>
      
      <!-- <el-table-column prop="otherFilePath" label="上传地址" align="center">
      </el-table-column> -->
...
export default {
  name: "qualityEvent",
  components: { Pagination,fileView,burstUpload },
  data() {
    return {
      loading: false,
      tableData: [], //当前分页数据
  ...
  methods: {
    ...
    huiFuLook(index, row) {
     console.log(row)
      let type = row.otherFilePath.substring(row.otherFilePath.lastIndexOf(".") + 1);
      this.$refs.fileView.show(row.otherFilePath, type); //(文件地址,文件类型)
    },
    handlepath(item) {
      if(item){
         let path = item.split("/");
    ...
    createData() {
      ...
      this.$refs.burstUpload.sureUploadVideo();
      return
    ...
    upUrlArr(data) {
      ...
      filePath = filePath.join(";");
        ...
        this.temp.otherFilePath = filePath;
        let param = this.temp;
        addTtUploadFile(param)
          .then((response) => {
            this.loading = false;
    ...
    huiFuUpload(index, row) {
      console.log(row.otherFilePath);
      if (row.otherFilePath) {
        var arr = [];
        arr.push(row.otherFilePath);
