<template>
  <div class="app-container">
      ...
      >
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="primary"
            @click="downloadFile(scope.row.otherFilePath)">下载</el-button>
          <el-button
            size="mini"
            type="danger"
    ...
    <!--增加或修改弹出层-->
    <el-dialog :title="textMap[dialogStatus]" :before-close="cancel" :visible.sync="dialogFormVisible" width="50%":close-on-click-modal="false">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="120px" style="margin-left:5px;">

        <!-- 单文件上传,无multiple -->
        <upload-file file-type="*" ref="clearData" list-type="*" accept="*" max-size="100"   @change="getFile"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      downloadFile(path){
        location.href="/wxgc-api/wxgc/plus/tt-file/downLoadFile?filePath="+path
      },
      cancel(){
        this.fileList = []
      ...
      getFile(file,fileList){
        ...
        for (let i = 0; i <fileList.length ; i++) {
          ...
          fd.append('createBy',this.$store.getters.realName)
        }
        this.fileList = fd
      },
      //单击添加选择文件
