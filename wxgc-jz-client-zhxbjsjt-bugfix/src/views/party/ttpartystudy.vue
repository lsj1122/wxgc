<template>
  <div class="app-container">
    ...
    >
      <el-table-column fixed prop="name" label="实践标题" align="center">
      </el-table-column>

      <el-table-column fixed prop="imgUrl" label="图片" align="center">
        ...
        <el-row>
          <el-col :span="24"
            ><div class="grid-content bg-purple">
              <el-form-item label="实践标题" prop="name">
                <el-input v-model="temp.name" placeholder="实践标题" />
              </el-form-item>
  ...
  methods: {
    ...
    deleteRow(index, rows) {
      this.$confirm("你确定要删除此信息?", "系统警告", {
        ...
        .then(async () => {
          let res = await deleteTtPartyStudy(rows[index].id);
          if (res.code == "success") {
            rows.splice(index, 1);
          }
          this.$message({
    ...
    loadPageData() {
      ...
      getPageTtPartyStudy(this.listQuery)
        .then((res) => {
          if (res.code == "success") {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          simpleUploads(this.fileList).then((res) => {
            ...
            let param = JSON.parse(JSON.stringify(this.temp));
            updateTtPartyStudy(param).then((response) => {
              this.loading = false;
              if (response.code == "success") {
                this.cancel();
                this.loadPageData();
