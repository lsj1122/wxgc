<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="latitude" label="幅度(m)" align="center">
      </el-table-column>
      <el-table-column prop="turn" label="回转(°)" align="center">
      </el-table-column>

      <el-table-column prop="weight" label="起重量(吨)" align="center">
      </el-table-column>
      ...
      <el-table-column prop="alermReason" label="报警类型" align="center">
        <template slot-scope="scope">
          {{ getAlermReason(scope.row.alermReason) }}
        </template>
      </el-table-column>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { label: "高度(m)", field: "height" },
        { label: "幅度(m)", field: "latitude" },
        { label: "回转(°)", field: "turn" },
        { label: "起重量(吨)", field: "weight" },
        { label: "风速(m/s)", field: "speed" },
        // { label: "起重量数据", field: "loadWeight" },
        { label: "倾角(°)", field: "lean" },
        { label: "重量百分比(%)", field: "weightPercent" },
        { label: "力矩百分比(%)", field: "momentPercent" },
        // {label: "力矩", field: "moment"},
        {label: "力矩百分比", field: "momentPercent"},
        { label: "制动状态", field: "brakingStatus" },
        { label: "设备状态", field: "alermReason" },
        ...
        .then((res) => {
          if (res.code == "success") {
            ...
            records.forEach((item) => {
              item.brakingStatus =
                item.brakingStatus == 0
                  ? "闭合"
                  : item.brakingStatus == 0
                  ? "断开"
                  : "";
              item.alermReason = this.getAlermReason(item.alermReason);
            });
