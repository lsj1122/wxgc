    ...
    >
      ...
      <el-table-column prop="innerDate" label="进场时间" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.innerDate.split(" ").shift() }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    deleteRow(index, rows) {
      this.$confirm("你确定要删除?", "系统警告", {
        ...
        .then(async () => {
          let res = await deleteYDJC(rows[index].id);
          if (res.code == "success") {
            rows.splice(index, 1);
          }
          this.$message({
    ...
    handleDownload() {
      ...
      getPageGDJC(params)
        .then((res) => {
          if (res.code == "success") {
            this.allTableData = res.data.records; //当前页数据
            this.allTableData.forEach((element) => {
              element.innerDate = element.innerDate.split(" ").shift();
            });
    ...
    updateData() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          console.log(param);
          updateYDJC(param).then((response) => {
            console.log(441, response);
            if (response.code == "success") {
              this.dialogFormVisible = false;
              this.loadPageData();
            }
