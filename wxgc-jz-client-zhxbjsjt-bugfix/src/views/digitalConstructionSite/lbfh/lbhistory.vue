...
import {getPageTtLbSs, getAllTdWorkData} from "./api/lbfh.js";
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {label: "线锁1状态", field: "wireLockOne"},
        {label: "线锁2状态", field: "wireLockTwo"},
        {label: "磁锁1状态", field: "magneticLockOne"},
        {label: "磁锁2状态", field: "magneticLockTwo"},
        {label: "电压(V)", field: "voltage"},
        {label: "上报时间", field: "uploadTime"},
      ],
      dataLoading: false,
      total: 0,
      listQuery: {
        ...
        .then((res) => {
          if (res.code == "success") {
            ...
            for (let i = 0; i < this.allTableData.length; i++) {
              this.allTableData[i].wireLockOne =
                this.allTableData[i].wireLockOne == 0
                  ? "正常"
                  : this.allTableData[i].wireLockOne == 1
                    ? "防护网拆除"
                    : "";
              this.allTableData[i].wireLockTwo =
                this.allTableData[i].wireLockTwo == 0
                  ? "正常"
                  : this.allTableData[i].wireLockTwo == 1
                    ? "防护网拆除"
                    : "";
              this.allTableData[i].magneticLockOne =
                this.allTableData[i].magneticLockOne == 0
                  ? "正常"
                  : this.allTableData[i].magneticLockOne == 1
                    ? "防护网拆除"
                    : "";
              this.allTableData[i].magneticLockTwo =
                this.allTableData[i].magneticLockTwo == 0
                  ? "正常"
                  : this.allTableData[i].magneticLockTwo == 1
                    ? "防护网拆除"
                    : "";
            }
