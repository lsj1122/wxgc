...
import { getPageRealTimeSZ } from "./api/comment.js";
...
export default {
  ...
  data() {
    return {
      ...
      //表头（主要是下载excel时候使用）
      columns: [
        { label: "设备序号", field: "machineOrderNumber" },
        { label: "设备编号", field: "machineNumber" },
        { label: "浊度值(NTU)", field: "turbidityValue" },
        { label: "ph", field: "phValue" },
        { label: "水位(m)", field: "waterLevelValue" },
        { label: "污泥(mg/L)", field: "sludgeValue" },
        { label: "氨氮值(ppm)", field: "ammoniaNitrogenValue" },
        { label: "温度(℃)", field: "temperature" },
        { label: "电导率(uS/cm)", field: "conductivity" },
  ...
  methods: {
    ...
    loadPageData() {
      ...
      getPageRealTimeSZ(this.listQuery)
        .then((res) => {
          if (res.code == "success") {
